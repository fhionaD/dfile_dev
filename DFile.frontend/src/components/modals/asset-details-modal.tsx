...
import { Package, Calendar, DollarSign, Tag, MapPin, Warehouse, Wrench, Building2, User, FileText } from "lucide-react";
            ...
            <DialogContent className="max-w-2xl rounded-2xl border-border p-0 overflow-hidden flex flex-col max-h-[85vh]">
                <DialogHeader className="p-6 bg-muted/40 border-b border-border shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <DialogTitle className="text-lg font-semibold text-foreground">{asset.desc}</DialogTitle>
                            <div className="flex items-center gap-2 mt-1.5">
                                <Badge variant="secondary" className="font-mono text-xs">{asset.id}</Badge>
                                <Badge variant="outline" className="text-xs">{asset.cat}</Badge>
                                <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${asset.status === "Available" ? "bg-sky-500/10 text-sky-700 border-sky-500/20" :
                                    asset.status === "In Use" ? "bg-emerald-500/10 text-emerald-700 border-emerald-500/20" :
                                        asset.status === "Maintenance" ? "bg-amber-500/10 text-amber-700 border-amber-500/20" :
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="bg-muted/30 rounded-xl p-4 space-y-3 border border-border/50">
                            <div className="flex items-center justify-between text-sm">
                                <span className="text-muted-foreground flex items-center gap-2"><DollarSign size={14} /> Value</span>
